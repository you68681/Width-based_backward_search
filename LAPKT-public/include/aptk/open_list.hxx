...
public:
	 ...
	 *  check later
	 */
    void 		insert_close( Node* );
    Node* 		pop_close();
    bool		empty_close() const;
    float		min_close() const;
    void		clear_close();
    Node*           top_close(){ return m_queue_close.top(); }

private:

	std::priority_queue< Node*, std::vector< Node* >, Node_Comp > m_queue;

    std::priority_queue< Node*, std::vector< Node* >, Node_Comp_close<Node> > m_queue_close;
};
 ...
 */
    ...
    template < typename Node_Comp, typename Node >
    void	Open_List<Node_Comp, Node>::insert_close( Node* n )
    {
        m_queue_close.push( n );
 ...
 */
    ...
    template <typename Node_Comp, typename Node >
    Node*	Open_List<Node_Comp, Node>::pop_close()
    {
        if( empty_close() ) return NULL;
        Node* elem = m_queue_close.top();
        m_queue_close.pop();
        return elem;
    }
 ...
 */
    ...
    template < typename Node_Comp, typename Node >
    bool	Open_List<Node_Comp, Node>::empty_close() const
    {
        return m_queue_close.empty();
    }
 ...
 */
    ...
    template < typename Node_Comp, typename Node >
    float     Open_List<Node_Comp, Node>::min_close() const
    {
        if ( empty_close() ) return 0.0f;
        return m_queue_close.top_close()->f;
    }
 ...
 */
    ...
    template < typename Node_Comp, typename Node >
    void	Open_List<Node_Comp, Node>::clear_close()
    {
        while ( !empty_close() )
        {
            Node* elem = pop_close();
