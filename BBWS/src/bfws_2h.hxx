...
template <typename bwd_Search_Model, typename Fwd_Search_Model, typename First_Heuristic, typename Second_Heuristic, typename Relevant_Fluents_Heuristic, typename Open_List_Type >
class BFWS_2H {

public:
	typedef	        typename bwd_Search_Model::State_Type		        State_bwd;
    typedef	        typename Fwd_Search_Model::State_Type		        State_fwd;
	typedef  	typename Open_List_Type::Node_Type		        Search_Node;
	typedef 	Closed_List< Search_Node >			        Closed_List_Type;
	typedef         aptk::agnostic::Landmarks_Graph_Manager<bwd_Search_Model,Fwd_Search_Model>   Landmarks_Graph_Manager;


	BFWS_2H( 	const bwd_Search_Model& search_problem ,const Fwd_Search_Model& search_problem_fwd )
	: m_problem( search_problem ), m_problem_fwd( search_problem_fwd ), m_exp_count(0), m_gen_count(0), m_dead_end_count(0), m_open_repl_count(0),m_max_depth( infty ), m_max_novelty(1), m_time_budget(infty), m_lgm(NULL), m_max_h2n(no_such_index), m_max_r(no_such_index), m_verbose( true ),  m_use_novelty(true), m_use_novelty_pruning(false), m_use_rp(true), m_use_rp_from_init_only(false) {
...
//            succ->print(std::cout);
	...
	const	bwd_Search_Model&	problem() const			{ return m_problem; }
    const	Fwd_Search_Model&	problem_fwd() const			{ return m_problem_fwd; }
...
protected:
	...
	const bwd_Search_Model&			m_problem;
    const Fwd_Search_Model&			m_problem_fwd;
