...
template <typename bwd_Search_Model, typename Fwd_Search_Model, typename Search_Node >
 ...
 * @param max_MB
 */
	Novelty_Partition_2( const bwd_Search_Model& prob_bwd, const Fwd_Search_Model& prob, unsigned max_arity = 1, const unsigned max_MB = 2048 )
	    : Heuristic<State>( prob_bwd,prob ), m_strips_model( prob.task() ), m_max_memory_size_MB(max_MB), m_always_full_state(false), m_verbose( true ), m_partition_size(0) {
...
protected:
	...
	bool cover_tuples( Search_Node* n, unsigned arity  )
	{
		...
		if(!has_state)
			n->parent()->state()->progress_lazy_state_fwd(  m_strips_model.actions()[ n->action() ] );
		...
		for( unsigned idx = 0; idx < n_combinations; idx++ ){
			 ...
			 * -> n better than old_n
			 */
			bool cover_new_tuple = ( !m_nodes_tuples_by_partition[ n->partition2() ][ tuple_idx ] ) ? true : ( is_better( m_nodes_tuples_by_partition[ n->partition2() ][tuple_idx], n  ) ? true : false);
			
			if( cover_new_tuple ){
				m_nodes_tuples_by_partition[ n->partition2() ][ tuple_idx ] = (Search_Node*) n;
		...
		if(!has_state)
			n->parent()->state()->regress_lazy_state_fwd( m_strips_model.actions()[ n->action() ] );
	...
	inline bool      is_better( Search_Node* n,const Search_Node* new_n ) const { 
		return new_n->is_better2( n );		
	        // bool ret = false;		
		// float n1 = new_n->h2n() + new_n->gn();
		// float n2 = n->h2n() + n->gn();
                // ret =  n1 < n2;
                // if(ret) return ret;
