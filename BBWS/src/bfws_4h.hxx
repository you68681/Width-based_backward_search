...
template <typename bwd_Search_Problem, typename Fwd_Search_Problem,typename First_Heuristic, typename Second_Heuristic, typename Third_Heuristic, typename Fourth_Heuristic, typename Open_List_Type >
class BFWS_4H {

public:

	typedef	        typename bwd_Search_Problem::State_Type		        State_bwd;
    typedef	        typename Fwd_Search_Problem::State_Type		        State_fwd;
	...
	BFWS_4H( 	const bwd_Search_Problem& search_problem ,const Fwd_Search_Problem& search_problem_fwd)
	: m_problem_bwd( search_problem), m_problem(search_problem_fwd),m_exp_count(0), m_gen_count(0), m_pruned_B_count(0),
	  m_dead_end_count(0), m_open_repl_count(0),m_B( infty ), m_time_budget(infty), m_lgm(NULL), m_max_h2n(no_such_index), m_max_h4n(no_such_index), m_verbose( true ), m_action2gen_nodes( search_problem.num_actions() ), m_use_novelty(true) {	
		m_first_h = new First_Heuristic( search_problem, search_problem_fwd);
		m_second_h = new Second_Heuristic( search_problem ,search_problem_fwd);
		m_third_h = new Third_Heuristic( search_problem,search_problem_fwd);
		m_fourth_h = new Fourth_Heuristic( search_problem,search_problem_fwd );
	}
...
//        m_root_edit = new Search_Node( m_problem.goal_state(), 0.0f, no_op, NULL, m_problem.num_actions() );
		...
		if(m_lgm){				
			m_lgm->apply_state_fwd( m_root->state()->fluent_vec(), m_root->land_consumed(), m_root->land_unconsumed() );
			eval(m_root);
			/** original
			 *
			 */
...
//	void	set_arity( float v, unsigned g ){ m_first_h->set_arity( v, g ); m_third_h->set_arity( v ); }
        void	set_arity( float v, unsigned g ){ m_first_h->set_arity_fwd( v, g ); }
	...
	bool	find_solution_forward( float& cost_fwd,std::vector<Action_Idx>& plan, Gen_Lms* graph=NULL) {
		...
		Search_Node* end = do_search();
		if ( end == NULL ) return false;
		extract_plan( m_root, end, plan, cost_fwd );
	...
	unsigned		generated_fwd() const		{ return m_gen_count; }
	void			inc_eval()			{ m_exp_count++; }
	unsigned		expanded_fwd() const		{ return m_exp_count; }
	...
	const	Fwd_Search_Problem&	problem() const			{ return m_problem; }
    const	bwd_Search_Problem&	problem_bwd() const			{ return m_problem_bwd; }
	...
	void	      eval( Search_Node* candidate, bool po = false ) {
		...
		if(m_lgm){
			...
			if (candidate->action() != no_op){
				...
				if( !candidate->has_state() && has_cond_eff ){
					candidate->parent()->state()->progress_lazy_state_fwd(  m_problem.task().actions()[ candidate->action() ] );

					m_lgm->apply_action_fwd( candidate->parent()->state(), candidate->action(), candidate->land_consumed(), candidate->land_unconsumed() );

					candidate->parent()->state()->regress_lazy_state_fwd( m_problem.task().actions()[ candidate->action() ] );
		...
		if(po){
			...
			candidate->set_po( po );
			m_second_h->eval_fwd( *(candidate->state()), candidate->h2n(), candidate->po() );
		}else
		    /** original
		     *
		     */
        {    m_second_h->eval_fwd( *(candidate->state()), candidate->h2n());
            ...
            if (candidate->state() == NULL) {
                ...
                deleted.clear();
                candidate->parent()->state()->progress_lazy_state_fwd(m_problem.task().actions()[candidate->action()]);
                m_second_h->eval_fwd(*(candidate->state()), candidate->h2n(), (candidate->parent()->state()->fluent_vec()));
                candidate->parent()->state()->regress_lazy_state_fwd(this->problem().task().actions()[candidate->action()],
                                                                 &added, &deleted);
            }
            if (candidate->state() != NULL)
                m_second_h->eval_fwd(*(candidate->state()), candidate->h2n(), candidate->state()->fluent_vec());
                **/
        }
...
//             m_second_h->eval( *(candidate->state()), candidate->h2n(),problem().init());
	...
	void			eval_po( Search_Node* candidate ) {
		 ...
		 *
		 */
		m_fourth_h->eval_fwd( *(candidate->state()), candidate->h4n(), candidate->po2() );
#ifdef DEBUG
        ...
        m_fourth_h->eval_4h_fwd( *(candidate->state()), candidate->h4n(), app_set,candidate->po2() );
#endif
 ...
 *
 */
//        m_fourth_h->eval_chao_fwd( *(candidate->state()),*(problem().goal_state()), candidate->h4n(), app_set, candidate->po2() );
		m_fourth_h->eval_chao_fwd( *(candidate->state()),*(problem().init_state()), candidate->h4n(), app_set, candidate->po2() );
